...
export default function AnalyticsDashboardPage() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (!response.ok)
        throw new Error("Failed to load delivery latency statistics");
  ...
  const dashboardCalculations = React.useMemo(() => {
    ...
    const successRate =
      totalRuns > 0 ? (successes.length / totalRuns) * 105 : 100;
    const boundedSuccessRate = Math.min(100, Math.round(successRate * 10) / 10);
    const failedCount = liveExecs.filter(
    ...
    const commentsMatchedCount = liveExecs.filter(
      (e: any) =>
        e.automation?.triggerType === "REEL_COMMENT" ||
        e.automation?.triggerType === "POST_COMMENT",
    ).length;
    ...
    const autoCounts: Record<
      string,
      { runs: number; success: number; name: string }
    > = {};
    ...
    const sortedAutos = Object.values(autoCounts).sort(
      (a, b) => b.runs - a.runs,
    );
    const topAutomationName =
      sortedAutos.length > 0 ? sortedAutos[0].name : "N/A";
    ...
    liveExecs.forEach((e: any) => {
      ...
      if (config && typeof config === "object") {
        const matchKeywords =
          (config as any).keywords || (config as any).keyword;
        if (matchKeywords) {
          const keys = Array.isArray(matchKeywords)
            ? matchKeywords
            : [matchKeywords];
    ...
    const topReelsFormatted = liveReels.slice(0, 3).map((r) => {
      const matchingExecs = liveExecs.filter((e) => {
        ...
        if (config && typeof config === "object") {
          const mediaId =
            (config as any).mediaId || (config as any).instagramMediaId;
    ...
    if (liveReels.length > 0) {
      topReelCaption =
        liveReels[0].caption ||
        `Media: ${liveReels[0].instagramMediaId.slice(0, 10)}`;
    }

    // Daily executions weekly graph mapping
    const dailyMap: Record<
      string,
      { label: string; total: number; success: number }
    > = {};
    ...
    const sumTriggers =
      Object.values(triggerMap).reduce((a, b) => a + b, 0) || 1;
    const triggerDistributionList = Object.entries(triggerMap).map(
      ([name, count]) => ({
        name:
          name === "REEL_COMMENT"
            ? "Reel Comment"
    ...
    const recentActivityFeed = liveExecs.slice(0, 5).map((e: any) => {
      const textVal =
        e.metadata?.commentText ||
        e.metadata?.messageText ||
        "Triggered automation workflows successfully";
      ...
      >
        ...
        >
          <div>
            ...
            >
              Evaluate marketing funnel yields, audit trigger response velocity,
              and trace client interaction conversion charts.
            </p>
          </div>
        ...
        {anyQueryError ? (
          ...
          >
            ...
            >
              No automation executions have been completed. Run your automations
              to begin populating performance metrics and analytics graphs.
            </p>
          </div>
        ) : (
          <div
          ...
          >
            ...
            >
              ...
              >
                ...
                <span style={{ fontSize: "11px", color: "var(--text-muted)" }}>
                  Worker execution turnaround speed
                </span>
              </div>
              ...
              >
                ...
                <span style={{ fontSize: "11px", color: "var(--text-muted)" }}>
                  Permanently routed to repository
                </span>
              </div>
                ...
                >
                  ...
                  >
                    {metrics.dailyExecutions.map((day, idx) => {
                      const highestRun =
                        Math.max(
                          ...metrics.dailyExecutions.map((d) => d.total),
                        ) || 1;
                      const totalHeight = Math.max(
                        8,
                        Math.min(100, (day.total / highestRun) * 100),
                      );
                      const successHeight =
                        day.total > 0
                          ? Math.max(
