...
const runValidation = (draft: AutomationDraft): { valid: boolean; warnings: string[] } => {
  ...
  draft.actions.forEach((act, index) => {
    if (act.actionType === 'SEND_MESSAGE') {
      ...
      if (delay === undefined || delay < 0) {
        warnings.push(`Action ${index + 1} (Wait) requires positive delay seconds.`);
      }
    }
  });
  ...
  persist(
    (set, get) => {
      const pushToUndo = (currentDraft: AutomationDraft) => {
        ...
        const nextUndo = [...undoStack, JSON.parse(JSON.stringify(currentDraft))].slice(-50);
...
export const useConditions = () => useBuilderStore((state) => state.draft.conditions);
