  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-6)" }}>
      ...
      <div>
        ...
        <Radio.Group
          value={mediaScope}
          onChange={(e: any) => handleUpdate({ mediaScope: e.target.value, mediaId: e.target.value === "ALL_POSTS" ? "" : mediaId })}
          size="middle"
        >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-2)" }}>
          ...
          {mediaScope === "SPECIFIC_POST" && !mediaId && (
            <span style={{ fontSize: 11, color: "var(--warning)", fontWeight: 500, marginTop: "4px" }}>
              * Select a specific feed post from the library to apply automation rules.
            </span>
          )}
        </div>
      )}
