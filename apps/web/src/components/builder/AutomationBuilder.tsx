    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', width: '100%' }}>
      ...
      >
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 'var(--space-4)', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-1)', flex: '2 1 300px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-1)', flex: '3 1 350px' }}>
            <span style={{ fontSize: 11, fontWeight: 650, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.04em' }}>
              Description Context
            </span>
            <Input
          ...
          {/* Undo/Redo Actions */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', alignSelf: 'flex-end', height: 40 }}>
            <button
              onClick={undo}
              disabled={!canUndo}
              style={{
          ...
          return (
            <React.Fragment key={index}>
              ...
              >
                <div
                  style={{
                    ...
                    height: 22,
                    borderRadius: '50%',
                    background: isCurrent ? 'var(--primary)' : isCompleted ? 'var(--hover-bg)' : 'var(--surface-secondary)',
                    color: isCurrent ? '#fff' : isCompleted ? 'var(--primary)' : 'var(--text-secondary)',
                    fontSize: 11,
                    fontWeight: 650,
                    display: 'flex',
                    alignItems: 'center',
              ...
              {index < stepsList.length - 1 && (
                <div style={{ flex: 1, height: 1, minWidth: 20, background: 'var(--border)' }} />
              )}
            </React.Fragment>
          );
        })}
      ...
      >
        <button
          ...
          onMouseEnter={(e) => { if (currentStep !== 0) e.currentTarget.style.background = 'var(--surface-secondary)'; }}
          onMouseLeave={(e) => { if (currentStep !== 0) e.currentTarget.style.background = 'var(--surface)'; }}
          type="button"
        >
          <ArrowLeft size={15} />
          Back
        ...
        {currentStep < 4 ? (
          ...
          <button
            ...
            style={{
              ...
              gap: 'var(--space-2)',
              padding: '10px var(--space-6)',
              background: draft.review.valid ? 'var(--success)' : 'var(--text-muted)',
              color: '#fff',
              border: 'none',
              borderRadius: 'var(--radius-md)',
              fontSize: 13,
            ...
            onMouseEnter={(e) => { if (draft.review.valid) e.currentTarget.style.background = 'var(--success)'; }}
            onMouseLeave={(e) => { if (draft.review.valid) e.currentTarget.style.background = 'var(--success)'; }}
            type="button"
          >
            <Save size={15} />
